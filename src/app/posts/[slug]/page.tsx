  ...
  if (!post) {
    return (
      <div className="pt-24 text-center">
        ...
        <Link href="/" className="text-jungle-400 hover:underline mt-4 inline-block">
          Go home
        </Link>
      </div>
    );
  ...
  return (
    <div className="pt-16">
      ...
      <article className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-10">
        ...
        >
          <Link
            href={`/trips/${post.categorySlug}`}
            className="inline-flex items-center gap-1.5 text-sm text-jungle-400 hover:text-jungle-300 transition-colors mb-6"
          >
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-slate-400 mb-4">
            ...
            <Link
              href={`/trips/${post.categorySlug}`}
              className="flex items-center gap-1.5 text-jungle-400 hover:text-jungle-300 transition-colors"
            >
            ...
            <div className="flex gap-2 overflow-x-auto pb-4 mb-8 scrollbar-thin">
              {post.images.map((img, i) => (
                <button
                  ...
                  className="flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden border-2 border-transparent hover:border-jungle-400 transition-colors"
                >
                  <img
                    src={img}
                    alt={`Photo ${i + 1}`}
          ...
          <div className="blog-content glass-card p-6 sm:p-8 md:p-10">
            {processedContent}
          </div>
          ...
          <div className="flex justify-between items-center mt-12 gap-4">
            {adjacent?.prev ? (
              <Link
                ...
                className="glass-card p-4 flex items-center gap-3 group flex-1 max-w-[45%]"
              >
                <ArrowLeft className="w-5 h-5 text-jungle-400 flex-shrink-0 group-hover:-translate-x-1 transition-transform" />
                <div className="min-w-0">
                  <span className="text-xs text-slate-500">Previous</span>
                  <p className="text-sm text-slate-300 group-hover:text-jungle-300 transition-colors truncate">
                    {adjacent.prev.title}
                  </p>
                </div>
              </Link>
            ...
            {adjacent?.next ? (
              <Link
                ...
                className="glass-card p-4 flex items-center gap-3 group flex-1 max-w-[45%] text-right"
              >
      ...
      <Lightbox
        ...
        onNext={() =>
          setLightboxIndex((i) => Math.min(post.images.length - 1, i + 1))
        }
      />
    </div>
  );
